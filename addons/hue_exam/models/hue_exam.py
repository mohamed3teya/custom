...
class HUEExam(models.Model):
    ...
    course_id = fields.Many2one('op.course',required=True, domain="[('id', 'in', suitable_course_ids)]")
    ...
    suitable_session_ids = fields.Many2many('op.exam.session', compute='_compute_suitable_course_ids',)
    subject_id = fields.Many2one('op.subject', 'Subject', required=True, domain="[('id', 'in', suitable_subject_ids)]")
    suitable_subject_ids = fields.Many2many('op.subject', compute='_compute_suitable_course_ids',)
    ...
    @api.onchange('session_id', 'course_id')
    def _compute_suitable_course_ids(self):
        for rec in self:
            ...
            course_subjects = self.env['op.subject'].sudo().search([('course_id', '=', rec.session_id.course_id.id)])
            subjects = []
