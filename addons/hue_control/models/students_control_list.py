...
class GradesStudentList(models.Model):
    _name = 'students.control.list'
    _description = 'students.control.list'
    
    
    connect_id = fields.Many2one('students.subject.control', string='Control', ondelete='cascade', required=True, index=True)
    ...
    @api.depends('grade_final', 'grade_quiz', 'grade_activity', 'grade_attendance', 'grade_pract', 'grade_pract2',
                 ...
                 'grade_eassy','grade_skilllab', 'grade_practical')
    def _get_total(self):
        for rec in self:
            ...
            if rec.connect_id.flag_grade_oral and not rec.grade_oral:
                rec.total = False
                continue
            if rec.connect_id.flag_grade_medterm and not rec.grade_medterm:
                rec.total = False
                continue
            ...
            total = grade_final + grade_quiz + grade_activity + grade_attendance + grade_pract + grade_pract2 + grade_oral + grade_medterm + grade_coursework + grade_clinic + grade_pract_oral + grade_mcq + grade_eassy + grade_skilllab + grade_practical
            if rec.connect_id.course_id.id == 15:
    ...
    def _get_grade(self):
        ...
        for rec in self:
            StuSubjectIDs = self.env['op.student.semesters.subjects'].sudo().search(
                    [('subject_id', '=', rec.connect_id.subject_id.id),('student_id', '=', rec.student_id.id),
                    ...
                    order='student_code')
            if  StuSubjectIDs.control_grade and rec.total == StuSubjectIDs.subject_degree:
                rec.control_degree = StuSubjectIDs.control_degree
                rec.control_grade = StuSubjectIDs.control_grade
                rec.grade = StuSubjectIDs.subject_grade
            else :
                if rec.total == 'غ':
...
class SubControlResultXls(models.Model):
    _name = 'students.control.subjects.assessment.xls'
    _description = 'students.control.subjects.assessment.xls'
